...
>
	...
	>
		<div class="w-max">
			<TextbookTooltip id="self-attention"
				>{isRwkv ? 'Multi-head State Mixing' : 'Multi-head Self Attention'}</TextbookTooltip
			>
		</div>
	</div>
				...
				>
					<div class="qkv flex h-full flex-col justify-center gap-[5rem] pl-[6rem]">
						<div class="column key">
							<div class="head1 title">
								<TextbookTooltip id="qkv">{isRwkv ? 'Previous' : 'Key'}</TextbookTooltip>
							</div>
							...
							<Tooltip class="popover" triggeredBy={'.step.attention .key .cell'} placement="right"
								>{isRwkv ? 'Previous state' : 'Key'}, Head {$attentionHeadIdx + 1},
								vector({$modelMeta.dimension / $modelMeta.attention_head_num})</Tooltip
							>
						</div>
						<div class="column query">
							<div class="head1 title">
								<TextbookTooltip id="qkv">{isRwkv ? 'Current' : 'Query'}</TextbookTooltip>
							</div>
							{#each $tokens as token, index}
								<div
							...
							<Tooltip
								...
								placement="right"
								>{isRwkv ? 'Current hidden state' : 'Query'}, Head {$attentionHeadIdx + 1},
								vector({$modelMeta.dimension / $modelMeta.attention_head_num})</Tooltip
							>
						</div>
						<div class="column value">
							<div class="head1 title">
								<TextbookTooltip id="qkv">{isRwkv ? 'Update' : 'Value'}</TextbookTooltip>
							</div>
							{#each $tokens as token, index}
							...
							<Tooltip
								...
								placement="right"
								>{isRwkv ? 'State update carrier' : 'Value'}, Head {$attentionHeadIdx + 1},
								vector({$modelMeta.dimension / $modelMeta.attention_head_num})</Tooltip
							>
						</div>
					</div>
					...
					<div class="head-out mx-[2rem]">
						<div class="column out">
							<div class="head1 title">
								<TextbookTooltip id="output-concatenation">{isRwkv ? 'State' : 'Out'}</TextbookTooltip>
							</div>
							{#each $tokens as token, index}
							...
							<Tooltip class="popover" triggeredBy={'.step.attention .out .cell'} placement="right"
								>{isRwkv ? 'State-mixed hidden' : 'Attention Out'}, Head 1,
								vector({$modelMeta.dimension / $modelMeta.attention_head_num})</Tooltip
							>
						</div>
					</div>
