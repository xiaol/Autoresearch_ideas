<script lang="ts">
	...
	$: qkvPaths = [
		{
			...
			onMouseOver: () => {
				...
				tooltip.set(
					$selectedTraceModel === 'rwkv'
						? 'RWKV streams: r, w, k~, k-, v, a, g'
						: 'click to see QKV calculation'
				);
			},
			onMouseOut: () => {
				const paths = d3.select(`g.qkv`).selectAll('path');
			...
			onMouseClick: (e, d) => {
				e.stopPropagation();
				if ($selectedTraceModel === 'rwkv') return;
	...
	$: attentionPaths = [
		...
		},
		{
			...
			onMouseOver: () => {
				...
				tooltip.set(
					$selectedTraceModel === 'rwkv'
						? 'RWKV readout: S*r plus current-token bonus'
						: 'click to see Attention Out calculation'
				);
			},
			onMouseOut: () => {
    ...
    `;
			...
			onMouseOver: () => {
				...
				tooltip.set(
					$selectedTraceModel === 'rwkv'
						? 'RWKV write/read contribution into the block output'
						: 'click to see Attention Out calculation'
				);
			},
			onMouseOut: () => {
	...
	$: mlpUpPaths = [
		{
			...
			onMouseOver: () => {
				...
				tooltip.set(
					$selectedTraceModel === 'rwkv'
						? 'RWKV Channel Mix: squared-ReLU key activation'
						: 'click to see MLP process'
				);
			},
			onMouseOut: () => {
				const paths = d3.select(`g.mlpUp`).selectAll('path');
	...
	$: mlpDownPaths = [
		{
			...
			onMouseOver: () => {
				...
				tooltip.set(
					$selectedTraceModel === 'rwkv'
						? 'RWKV Channel Mix: value projection'
						: 'click to see MLP process'
				);
			},
			onMouseOut: () => {
