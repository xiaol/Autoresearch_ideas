<script lang="ts">
	...
	const onClickNext = (e) => {
		e.stopPropagation();
		textPages
			.find((page) => page.id === ($selectedTraceModel === 'rwkv' ? 'rwkv-blocks' : 'blocks'))
			?.complete?.();
...
<div
	...
	class:active={$isBoundingBoxActive}
>
	<TextbookTooltip id={$selectedTraceModel === 'rwkv' ? 'rwkv-blocks' : 'blocks'}>
		<span class="title-text"
			>{$selectedTraceModel === 'rwkv' ? 'RWKV Block' : 'Transformer Block'} {$blockIdxTemp + 1}</span
		>
	</TextbookTooltip>
	<button
