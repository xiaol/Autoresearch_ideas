<script lang="ts">
	...
	import RwkvStateCalculationPopover from './Popovers/RwkvStateCalculationPopover.svelte';
	...
	onMount(() => {
		const setPosition = () => {
			...
			const rwkvState = document.querySelector('.step.rwkv-state .attention-matrix-container');
			...
			const rwkvStateRect = rwkvState?.getBoundingClientRect();
			...
			rwkvStatePos = {
				left: (rwkvStateRect?.right || 0) + scrollLeft,
				top: (rwkvStateRect?.top || 0) - topbarHeight
			};
			softmaxPos = {
...
{/if}
{#if $weightPopover === 'rwkvState'}
	<div
		bind:this={popoverEl}
		class="rwkv-state-weight-popover weight-popover"
		style={`left:${rwkvStatePos.left}px;top:${rwkvStatePos.top}px;`}
		in:fade={{ duration: 300 }}
		role="group"
	>
		<RwkvStateCalculationPopover></RwkvStateCalculationPopover>
	</div>
{/if}
...
<style lang="scss">
	...
	.rwkv-state-weight-popover {
		transform: translate(calc(-100% - 1rem), -15%);
	}
	.mlp-weight-popover {
