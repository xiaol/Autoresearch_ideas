<script lang="ts">
	...
	import {
		getTextPagesForModel,
		mapTextPageIdForModel,
		type TextbookPage
	} from '~/utils/textbookPages';
	import TextbookCard from './TextbookCard.svelte';

	export let showTextCard: boolean = false;

	let activeTextPages: TextbookPage[] = [];
	let activePage: TextbookPage | undefined;
	let lastActivePage: TextbookPage | undefined;
	let lastTraceModel: TraceModelKey | undefined;

	$: activeTextPages = getTextPagesForModel($selectedTraceModel);
	$: activePage = activeTextPages[$textbookCurrentPage] || activeTextPages[0];

	$: if (activeTextPages.length) {
		const modelChanged = lastTraceModel && lastTraceModel !== $selectedTraceModel;
		const mappedPageId = modelChanged
			? mapTextPageIdForModel($textbookCurrentPageId, $selectedTraceModel)
			: $textbookCurrentPageId;
		const pageIndex = activeTextPages.findIndex((page) => page.id === mappedPageId);

		if (pageIndex === -1 || pageIndex !== $textbookCurrentPage) {
			const nextPageIndex = pageIndex === -1 ? 0 : pageIndex;
			textbookPreviousPageId.set($textbookCurrentPageId);
			textbookPreviousPage.set($textbookCurrentPage);
			textbookCurrentPageId.set(activeTextPages[nextPageIndex]?.id || '');
			textbookCurrentPage.set(nextPageIndex);
		}
		lastTraceModel = $selectedTraceModel;
	}

	$: if (showTextCard && activePage && lastActivePage !== activePage) {
		lastActivePage?.out?.();
		setTimeout(() => {
			activePage?.on?.();
		}, 0);
	...
	$: if (!showTextCard && lastActivePage) {
		lastActivePage.out?.();
...
{#if showTextCard}
	<TextbookCard textPages={activeTextPages} onClose={() => isTextbookOpen.set(false)} />
{/if}
