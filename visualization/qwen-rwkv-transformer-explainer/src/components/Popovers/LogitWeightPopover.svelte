<script lang="ts">
	...
	const tokenGap = 6;
	$: vocabSizeLabel = ($modelMeta.vocab_size || 151936).toLocaleString();
...
<WeightPopoverCard id="logits" title={'Logits'} bind:isAnimationActive {timeline} bind:isOpen>
	<div class="weight-popover-content flex items-center justify-start">
		...
		<div class="matrix flex flex-col items-center">
			...
			<div class="size">({$modelMeta.dimension}, {vocabSizeLabel})</div>
		</div>
		...
		<div class="matrix flex flex-col items-center">
			...
			/>
			<div class="size">({vocabSizeLabel})</div>
		</div>
		...
		<div class="matrix flex flex-col items-center">
			...
			<div class="size">(1, {vocabSizeLabel})</div>
		</div>
	</div>
	<div class="formula">
