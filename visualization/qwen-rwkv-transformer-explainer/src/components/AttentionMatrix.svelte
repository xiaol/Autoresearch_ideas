<script lang="ts">
	...
	$: isRwkv = $selectedTraceModel === 'rwkv';
	const attentionDataKey = (blockIndex: number, headIndex: number, suffix: string) =>
		`block_${blockIndex}_attn_head_${headIndex}_${suffix}`;
	const resolveAttentionBlockIdx = (
		outputs: ModelData['outputs'] | undefined,
		blockIndex: number,
		headIndex: number
	) => {
		const modelOutputs = outputs || {};
		if (modelOutputs[attentionDataKey(blockIndex, headIndex, 'attn_masked')]?.data) return blockIndex;

		const available = Object.keys(modelOutputs)
			.map((key) => key.match(new RegExp(`^block_(\\d+)_attn_head_${headIndex}_attn_masked$`)))
			.filter((match): match is RegExpMatchArray => !!match)
			.map((match) => Number(match[1]))
			.sort((a, b) => a - b);

		if (!available.length) return blockIndex;
		return available.reduce((best, candidate) => {
			const bestDistance = Math.abs(best - blockIndex);
			const candidateDistance = Math.abs(candidate - blockIndex);
			if (candidateDistance < bestDistance) return candidate;
			return best;
		}, available[0]);
	};
	$: attentionBlockIdx = resolveAttentionBlockIdx($modelData?.outputs, $blockIdx, $attentionHeadIdx);
	$: isAttentionBlockFallback = !isRwkv && attentionBlockIdx !== $blockIdx;
	$: queryKey =
		$modelData?.outputs?.[attentionDataKey(attentionBlockIdx, $attentionHeadIdx, 'attn')]?.data ||
		placeHolderData;
	$: masked =
		$modelData?.outputs?.[attentionDataKey(attentionBlockIdx, $attentionHeadIdx, 'attn_masked')]?.data ||
		placeHolderData;
	$: softmaxed =
		$modelData?.outputs?.[attentionDataKey(attentionBlockIdx, $attentionHeadIdx, 'attn_dropout')]
	...
	$: maskedColorScale = (d, i) => {
		if (isRwkv) return d3.interpolate('white', theme.colors['purple'][700])(d);
	...
	>
		...
		>
			...
			<TextbookTooltip id="masked-self-attention">
				<div class="matrix-label">{isRwkv ? 'Raw similarity' : 'Dot product'}</div>
			</TextbookTooltip>

			<Tooltip class="popover tooltip">
				<Katex math={isRwkv ? 'h_i \\cdot h_j' : 'Q \\cdot K^T'}></Katex>
			</Tooltip>
		...
		>
			...
			<TextbookTooltip id="masked-self-attention">
				<div class="matrix-label">{isRwkv ? 'Causal mask' : 'Scaling · Mask'}</div>
			</TextbookTooltip>

			<Tooltip class="popover tooltip">
				<Katex math={isRwkv ? 'j \\le i' : '\\frac{QK^T}{\\sqrt{d_k}} + M'}></Katex>
			</Tooltip>
			<div class="color-scale">
				<span class="val">{isRwkv ? '0.0' : '-3.0'}</span>
				<div class="bar"></div>
				<span class="val">{isRwkv ? '1.0' : '3.0'}</span>
			</div>
		</div>
		...
		>
			...
			<TextbookTooltip id="masked-self-attention">
				<div class="matrix-label">{isRwkv ? 'Normalized' : 'Softmax'}</div>
			</TextbookTooltip>
			<Tooltip class="popover tooltip">
				<Katex
					math={isRwkv
						? '\\frac{\\max(0,\\cos(h_i,h_j))}{\\sum_{k\\le i}\\max(0,\\cos(h_i,h_k))}'
		...
		>
			...
			<div class="matrix-label flex items-center gap-1">
				{isRwkv ? 'State Similarity' : 'Attention'} <ZoomInOutline></ZoomInOutline>
			</div>
			{#if isAttentionBlockFallback}
				<div class="matrix-note">full-attn block {attentionBlockIdx + 1}</div>
			{/if}
		</div>
	</div>
</div>
