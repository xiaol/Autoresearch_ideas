...
const RWKV_TRACE_VERSION = 'rwkv-state-matrix-v3';

const modelKeyFromTrace = (traceModel: TraceModelKey, modelId = '') => {
	if (traceModel === 'rwkv') return 'rwkv7-g1-0.1b';
	if (modelId.includes('Qwen3.5-0.8B')) return 'qwen3.5-0.8b';
	return 'qwen3-0.6b';
};

const canUseStaticTrace = ({
	temperature,
	sampling
}: {
	temperature: number;
	...
	Math.abs(temperature - initialTemperature) < 1e-6 &&
	sampling.type === 'top-k' &&
	sampling.value === 5;

const traceLabel = (traceModel: TraceModelKey) => (traceModel === 'rwkv' ? 'RWKV' : 'Qwen');

const hasRwkvStateTransition = (trace: TracePayload) => {
	const stateAfter = trace.rwkvBlocks?.[0]?.stateTransition?.stateAfter;
	if (!Array.isArray(stateAfter)) return false;

	return stateAfter.some((tokenMatrices) =>
		tokenMatrices?.some((headMatrix) =>
			headMatrix?.some((row) =>
				row?.some((value) => Number.isFinite(value) && Math.abs(value) > 1e-12)
			)
		)
	);
};

const fetchStaticTrace = async (
	traceModel: TraceModelKey,
	exampleIndex: number
): Promise<TracePayload | null> => {
	const traceDir = traceModel === 'rwkv' ? 'rwkv-traces' : 'qwen-traces';
	const version = traceModel === 'rwkv' ? `?v=${RWKV_TRACE_VERSION}` : '';
	const response = await fetch(`${base}/${traceDir}/example-${exampleIndex}.json${version}`, {
		cache: traceModel === 'rwkv' ? 'no-store' : 'default'
	});
	if (!response.ok) return null;
	return response.json();
};

const fetchServerTrace = async ({
	traceModel,
	input,
	temperature,
	sampling
	...
	const response = await fetch(`${base}/api/${traceModel}-trace`, {
		method: 'POST',
	...
	if (!response.ok) {
		...
		throw new Error(text || `${traceLabel(traceModel)} trace request failed with ${response.status}`);
	}
...
const getTrace = async ({
	...
	if (exampleIndex !== undefined && canUseStaticTrace({ temperature, sampling })) {
		try {
			const trace = await fetchStaticTrace(traceModel, exampleIndex);
			if (traceModel === 'rwkv' && trace && !hasRwkvStateTransition(trace)) {
				console.warn('Static RWKV trace is missing state-transition matrices; falling back.');
				return fetchServerTrace({ traceModel, input, temperature, sampling });
			}
			if (trace) return trace;
		} catch (error) {
			console.warn(`Static ${traceLabel(traceModel)} trace unavailable; falling back to server trace.`, error);
		}
	}
...
const applyTrace = (trace: TracePayload, traceModel: TraceModelKey) => {
	const modelKey = trace.modelKey || modelKeyFromTrace(traceModel, trace.modelId);
	if (trace.modelMeta && !modelMetaMap[modelKey]) {
		modelMetaMap[modelKey] = trace.modelMeta;
	}

	selectedModel.set(modelKey);
	tokens.set(trace.tokens);
	tokenIds.set(trace.tokenIds);
	modelData.set({
		logits: trace.logits || [],
		outputs: trace.outputs || {},
		probabilities: trace.probabilities || [],
		sampled: trace.sampled,
		rwkvState: trace.rwkvState,
		rwkvBlocks: trace.rwkvBlocks
	});
	predictedToken.set(trace.sampled);
	rwkvStateTokenIdx.set(null);

	const meta = trace.modelMeta || modelMetaMap[modelKey];
	if (meta) {
		blockIdx.update((value) => Math.min(value, meta.layer_num - 1));
		attentionHeadIdx.update((value) => Math.min(value, meta.attention_head_num - 1));
		blockIdxTemp.update((value) => Math.min(value, meta.layer_num - 1));
		attentionHeadIdxTemp.update((value) => Math.min(value, meta.attention_head_num - 1));
	}
};
...
export const runTrace = async ({
	...
	sampling,
	exampleIndex,
	traceModel = get(selectedTraceModel)
}: {
	input: string;
	temperature: number;
	sampling: Sampling;
	...
	try {
		...
		if (requestId === traceRequestId) {
			console.error(`Error loading ${traceLabel(traceModel)} trace:`, error);
			isModelRunning.set(false);
		}
	}
};

export const runQwenTrace = async (args: {
	input: string;
	temperature: number;
	sampling: Sampling;
	exampleIndex?: number;
}) => runTrace({ ...args, traceModel: 'qwen' });
