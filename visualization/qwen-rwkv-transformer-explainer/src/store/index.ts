...
import { qwenModelMetaMap, qwenPlaceholderData, qwenTraceExamples } from '~/constants/qwen';
import { rwkvModelMetaMap } from '~/constants/rwkv';
...
export const inputTextExample = qwenTraceExamples;

export const traceModelOptions = [
	{ key: 'qwen', label: 'Qwen 0.8B', modelKey: 'qwen3.5-0.8b' },
	{ key: 'rwkv', label: 'RWKV-7 0.1B', modelKey: 'rwkv7-g1-0.1b' }
] as const;
export const selectedTraceModel = writable<TraceModelKey>('qwen');
...
export const modelData = writable<ModelData>(qwenPlaceholderData);
export const predictedToken = writable<Probability>();
export const tokens = writable<string[]>(['Data', ' visualization', ' empowers', ' users', ' to']);
export const tokenIds = writable<number[]>([]);

export const modelMetaMap: Record<string, ModelMetaData> = {
	...qwenModelMetaMap,
	...rwkvModelMetaMap,
	gpt2: { layer_num: 12, attention_head_num: 12, dimension: 768, vocab_size: 50257, chunkTotal: 63 },
	'gpt2-medium': { layer_num: 24, attention_head_num: 16, dimension: 1024, vocab_size: 50257 },
	'gpt2-large': { layer_num: 36, attention_head_num: 20, dimension: 1280, vocab_size: 50257 }
};
...
const initialSelectedModel = 'qwen3.5-0.8b';
...
export const rwkvStateTokenIdx = writable<number | null>(null);
export const rwkvStateViewMode = writable<'matrix' | 'manifold'>('matrix');
