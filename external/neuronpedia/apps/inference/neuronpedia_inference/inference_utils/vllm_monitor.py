...
Usage:
    from neuronpedia_inference.inference_utils.vllm_monitor import VLLMMonitor, get_health_stats

    # Get current stats
    stats = await get_health_stats()
    
    # Or use the monitor instance
    monitor = get_monitor()
...
class VLLMMonitor:
    ...
    def _get_vllm_engine_stats_sync(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(self._model, VLLMSteerModel):
                return stats
            
            # Get the inner steerllm model
            inner = self._model._inner
...
async def get_health_stats(model=None) -> VLLMStats:
    """
    Convenience function to get health stats.
    
    Args:
        model: Optional VLLMSteerModel to use
...
async def log_health_stats(model=None):
