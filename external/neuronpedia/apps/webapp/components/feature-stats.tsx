          ...
          >
            ...
            <Tooltip.Provider delayDuration={300} skipDelayDuration={0}>
              <Tooltip.Root>
                ...
                <Tooltip.Portal>
                  <Tooltip.Content
                    className="z-20 max-w-[320px] rounded-md border bg-slate-50 px-3 py-3 text-left text-xs text-slate-700 shadow-lg sm:max-w-[420px]"
                    sideOffset={5}
                  >
                    {`This dashboard is generated by testing the feature with text snippets from the specified dataset. The number of total tokens tested TOTAL_NUM_TOKENS is NUM_PROMPTS * TOKENS_PER_PROMPT. `}
                    <br />
                    <br />
                    <strong>Activations Density</strong>
                    {`: Percentage of tokens that had activation greater than 0. For example, if we tested 16,364 NUM_PROMPTS and 128 TOKENS_PER_PROMPT, we would have a TOTAL_NUM_TOKENS of 2,097,152. If 5,000 of tested tokens had activation greater than 0, the Activations Density is 5,000 / 2,097,152 = 0.238%. A high activations density (eg 90%+) means this feature activated on most tokens. A zero means no tested tokens were activated - it's possibly a dead feature.`}
                    <br />
                    <br />
                    <strong>Top Graph</strong>: Histogram of non-zero token activations from the tested prompts. The x
                    axis is the activation value buckets, and the y axis is the number of tokens that had that
                    activation value. Each bar represents a range of activation values.{' '}
                    {activationMarkerValue !== undefined
                      ? `The green line indicates the activation value of this feature with your current prompt.`
                      : ''}
                    {currentNeuron?.logits_hist_data_bar_values.length > 0 ? (
                      <>
                        <br />
                        <br />
                        <strong>Bottom Graph</strong>
                        {`: Logit effects histogram, based on the vocabulary of the model. The x axis is how positively or negatively this feature affects each token in the model's vocabulary. The y axis is the number of unique tokens from the model's vocabulary. Each bar represents a range of logit effects. The sum of all y values equals the model's vocabulary size.`}
                      </>
                    ) : (
                      ''
                    )}
          ...
          <div className="flex flex-col sm:flex-col sm:gap-y-1 sm:pt-1">
                ...
                data={[
                  {
                    ...
                    hovertemplate: currentNeuron?.freq_hist_data_bar_values.map((val, i) => {
                      const values = currentNeuron?.freq_hist_data_bar_values;
                      const binWidth =
                        values.length > 1
                          ? i < values.length - 1
                            ? values[i + 1] - values[i]
                            : values[i] - values[i - 1]
                          : val;
                      const rangeStart = (val - binWidth / 2).toFixed(2);
                      const rangeEnd = (val + binWidth / 2).toFixed(2);
                      const count = currentNeuron?.freq_hist_data_bar_heights[i];
                      return `<b>Activation Value Range (x)</b>: ${rangeStart} – ${rangeEnd}<br><b># Activated Tokens (y)</b>: ${count.toLocaleString()}<extra></extra>`;
                    }),
                  },
                ]}
                layout={{
                  ...
                  shapes:
                    activationMarkerValue !== undefined
                      ? [
                          {
                            type: 'line',
                            x0: activationMarkerValue,
                            x1: activationMarkerValue,
                            y0: 0,
                            y1: Math.max(...(currentNeuron?.freq_hist_data_bar_heights || [])),
                  ...
                  annotations:
                    activationMarkerValue !== undefined
                      ? [
                          {
                            x: activationMarkerValue,
                            y: Math.max(...(currentNeuron?.freq_hist_data_bar_heights || [])),
                            text: `${activationMarkerValue.toFixed(1)}`,
                            showarrow: false,
            ...
            {currentNeuron?.logits_hist_data_bar_values.length > 0 && (
              <Plot
                ...
                data={(() => {
                  const allValues = currentNeuron?.logits_hist_data_bar_values || [];
                  const allHeights = currentNeuron?.logits_hist_data_bar_heights || [];
                  const binWidth =
                    allValues.length > 1 ? Math.abs(allValues[1] - allValues[0]) : allValues[0] ? allValues[0] : 1;

                  const makeLogitHoverTemplate = (val: number, height: number) => {
                    const rangeStart = (val - binWidth / 2).toFixed(2);
                    const rangeEnd = (val + binWidth / 2).toFixed(2);
                    return `<b>Logit Effect Range (x)</b>: ${rangeStart} – ${rangeEnd}<br><b>Vocab Tokens (y)</b>: ${height.toLocaleString()}<extra></extra>`;
                  };

                  const posIndices = allValues.map((v, i) => (v >= 0 ? i : -1)).filter((i) => i !== -1);
                  const negIndices = allValues.map((v, i) => (v < 0 ? i : -1)).filter((i) => i !== -1);

                  return [
                    {
                      x: posIndices.map((i) => allValues[i]),
                      y: posIndices.map((i) => allHeights[i]),
                      type: 'bar',
                      mode: 'lines+markers',
                      name: '',
                      marker: { color: 'rgba(16, 185, 129, 1.0)' },
                      hovertemplate: posIndices.map((i) => makeLogitHoverTemplate(allValues[i], allHeights[i])),
                    },
                    {
                      x: negIndices.map((i) => allValues[i]),
                      y: negIndices.map((i) => allHeights[i]),
                      type: 'bar',
                      name: '',
                      mode: 'lines+markers',
                      marker: { color: 'rgba(250, 128, 128, 1.0)' },
                      hovertemplate: negIndices.map((i) => makeLogitHoverTemplate(allValues[i], allHeights[i])),
                    },
                  ];
                })()}
                layout={{
